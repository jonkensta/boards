...
# KiCad 10 does not inherit the base flags into a variant: a variant entry that leaves in_pos_files out means "in
# the position files", so hand-soldered SW1 repeats in_pos_files no in its "vib" entry (checked with pos --variant vib)

s = Schematic('node', root_uuid_of(OUT), 'Net node: RP2040, 4 neighbour links, 4 corner RGB LEDs, piezo, sensor',
...
C0402 = {'C10', 'C11', 'C13', 'C15', 'C19', 'C23', 'C24', 'C25', 'C26'}   # + the corner-LED and LED-buffer caps
...
def passive(kind, x, y, value, desc=None, rot=0, ref=None, **kw):
    """Device:R / Device:C at (x, y). rot 0: pin 1 top, pin 2 bottom; rot 90: pin 1 left, pin 2 right.
    `ref` pins the designator (parts added later keep the existing numbering stable); else the next free number."""
    if ref is None:
        _n[kind] += 1
        ref = f'{kind}{_n[kind]}'
...
GPIO = {'2': 'LINK_N', '3': 'LINK_E', '4': 'LINK_S', '5': 'LINK_W', '6': 'LED_DATA',
...
for i, (name, x) in enumerate(zip(['LINK_N', 'LINK_E', 'LINK_S', 'LINK_W'], [150, 174, 198, 222])):
    ...
    Jn = s.place('Connector_Generic', 'Conn_01x03', f'J{i + 1}', g(x), g(y), rot=270, value=name[-1],
                 footprint=JST3_FP, fields=JST3, description='JST XH 3-pin, side entry', in_pos_files=False,   # hand-soldered THT
...
for pin, name in {'2': 'SWDIO', '4': 'SWCLK', '6': 'RUN', '8': 'BOOT'}.items():
...
# jig USB 5 V -> D6 Schottky -> +5V: USB can power a node, a powered net cannot backfeed the USB host
D6 = s.place('Device', 'D_Schottky', 'D6', g(JX + 8), g(JY + 9), rot=270, value='B5819W', footprint='Diode_SMD:D_SOD-123',
             description='Schottky 40 V 1 A, SOD-123 (jig USB 5 V -> +5V, no backfeed)',
             fields={'MPN': 'B5819W SL', 'Manufacturer': 'CJ', 'LCSC': 'C8598'}, prop_pos={'Reference': (3.0, -1.27), 'Value': (4.5, 1.27)})
W((JX + 4, JY + 4), (JX + 4, JY + 12), (JX + 8, JY + 12))
assert D6['1'] == (g(JX + 8), g(JY + 6)) and D6['2'] == (g(JX + 8), g(JY + 12))
s.wire(D6['1'], (g(JX + 8), g(JY + 4))); s.power('+5V', g(JX + 8), g(JY + 4))
# BOOT: 1k between QSPI_SS and the BOOT pad (RP2040 HDG: lets QSPI_SS over-drive a grounded BOOT)
_, rb = passive('R', g(JX + 22), g(JY - 12), '1k', rot=90, ref='R19', prop_pos={'Reference': (0, -2.2), 'Value': (0, 2.2)})
stub_label(rb['1'], 'QSPI_SS', 'l', length=3); stub_label(rb['2'], 'BOOT', 'r', length=3)
...
W((LX - 22, LY + 8), (LX + 12, LY + 8)); s.wire(U4['2'], (g(LX), g(LY + 8))); J(LX, LY + 8); J(LX - 14, LY + 8)
s.power('GND', g(LX), g(LY + 8)); s.flag(g(LX - 18), g(LY + 8)); J(LX - 18, LY + 8)
# D5: unidirectional 5 V TVS on +5V at the LDO input (hot-plug ringing on the cable inductance + ceramic C17)
D5 = s.place('Device', 'D_Zener', 'D5', g(LX - 22), g(LY + 3), rot=270, value='SMF5.0A', footprint='Diode_SMD:D_SOD-123F',
             description='TVS unidirectional 5 V standoff, 6.4 V min breakdown, 200 W, SOD-123F',
             fields={'MPN': 'SMF5.0A', 'Manufacturer': 'MDD', 'LCSC': 'C193402'}, prop_pos={'Reference': (-4.0, -1.27), 'Value': (-5.0, 1.27)})
assert D5['1'] == (g(LX - 22), g(LY)) and D5['2'] == (g(LX - 22), g(LY + 6))
W((LX - 14, LY - 2), (LX - 22, LY - 2), (LX - 22, LY)); W((LX - 22, LY + 6), (LX - 22, LY + 8))

# ---- D1..D4 corner LEDs (WS2812B-2020-V6 at +5V), U5 5 V data buffer -------------------------------
# One LED per board corner, a 2x2 sub-pixel grid (drawn here in the board's arrangement). Chain = quadrant order:
# LED_DIN -> D1 NE -> D2 NW -> D3 SW -> D4 SE (pixel index 0..3). The LEDs run from +5V directly and U5
# (74AHCT1G125, TTL input: VIH 2.0 V) level-shifts GPIO4's 3.3 V data to 5 V: VIH = 0.55 VDD is 2.89 V at 5.25 V.
EX, EY = 226, 96
s.text('Corner LEDs, chain D1 NE -> D2 NW -> D3 SW -> D4 SE (pixel 0..3), all on +5V. U5 (AHCT, VIH 2.0 V) shifts the 3.3 V data to 5 V: WS2812B-2020-V6 VIH = 0.55 VDD.', g(178), g(72))
LED_FIELDS = {'MPN': 'WS2812B-2020-V6', 'Manufacturer': 'Worldsemi', 'LCSC': 'C52917434'}   # C965555 (original 2020) is discontinued
LED_PP = {'Reference': (-5.0, -7.5), 'Value': (0.0, 10.5)}
UBX = EX - 30
U5 = s.place('74xGxx', '74AHCT1G125', 'U5', g(UBX), g(EY), value='74AHCT1G125', footprint='Package_TO_SOT_SMD:SOT-23-5',
             description='Single buffer, TTL input, 3-state, SOT-23-5 (3.3 V -> 5 V LED data)',
             fields={'MPN': 'SN74AHCT1G125DBVR', 'Manufacturer': 'Texas Instruments', 'LCSC': 'C7484'},
             prop_pos={'Reference': (6.0, -9.0), 'Value': (6.0, 9.0)})
assert U5['1'] == (g(UBX), g(EY - 8)) and U5['2'] == (g(UBX - 12), g(EY)) and U5['4'] == (g(UBX + 10), g(EY))
assert U5['5'] == (g(UBX - 4), g(EY - 8)) and U5['3'] == (g(UBX - 4), g(EY + 8))
stub_label(U5['2'], 'LED_DATA', 'l'); stub_label(U5['4'], 'LED_DIN', 'r')
W((UBX - 4, EY - 8), (UBX - 4, EY - 12)); s.power('+5V', g(UBX - 4), g(EY - 12))
_, cb = passive('C', g(UBX - 10), g(EY - 9), *C100, ref='C26')
W((UBX - 4, EY - 12), (UBX - 10, EY - 12)); s.wire((g(UBX - 10), g(EY - 12)), cb['1']); J(UBX - 4, EY - 12)
s.power('GND', *cb['2'])
W((UBX, EY - 8), (UBX, EY - 10), (UBX + 4, EY - 10)); s.power('GND', g(UBX + 4), g(EY - 10))       # /OE low: always driving
s.power('GND', *U5['3'])
chain = ['LED_DIN', 'LED_CH1', 'LED_CH2', 'LED_CH3', None]
for i, (ref, dx, dy, cref) in enumerate((('D1', 30, 0, 'C19'), ('D2', 0, 0, 'C23'), ('D3', 0, 22, 'C24'), ('D4', 30, 22, 'C25'))):
    x, y = EX + dx, EY + dy
    D = s.place('LED', 'WS2812B-2020', ref, g(x), g(y), value='WS2812B-2020-V6', fields=LED_FIELDS, prop_pos=LED_PP)
    assert D['3'] == (g(x - 6), g(y)) and D['4'] == (g(x), g(y - 6)) and D['1'] == (g(x + 6), g(y)) and D['2'] == (g(x), g(y + 6))
    stub_label(D['3'], chain[i], 'l')
    if chain[i + 1]:
        stub_label(D['1'], chain[i + 1], 'r')
    else:
        s.no_connect(*D['1'])
    W((x, y - 6), (x, y - 10)); s.power('+5V', g(x), g(y - 10))
    _, c = passive('C', g(x + 5), g(y - 7), *C100, ref=None if cref == 'C19' else cref, prop_pos={'Reference': (3.2, -1.9), 'Value': (3.6, 1.9)})
    W((x, y - 10), (x + 5, y - 10)); J(x, y - 10)
    s.power('GND', *c['2']); s.power('GND', *D['2'])
...
SW1 = s.place('Switch', 'SW_SPST', 'SW1', g(VX), g(VY), rot=90, value='SW-18010P', footprint='boards:SW-18010P',
              ...
              dnp=True, variants={'vib': {'dnp': False, 'in_pos_files': False}}, in_pos_files=False, prop_pos={'Reference': (-9.0, -1.27), 'Value': (-9.5, 1.27)})
